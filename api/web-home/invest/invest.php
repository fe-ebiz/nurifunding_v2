<?php
    ...
    if($goods["state"] == "N" || $goods["liiv"] == "N") {
		jsMsg('투자가 종료된 상품입니다.', './list.php');
    }
...
<script>
	...
	function app_submit() {
        var u_cash = Number($("input[name='use_cash']").val().replace(',', ''));
		...
		if(u_cash*10000 < 10000) {
			alert("투자금액은 최소 1만원 입니다.");
			return false;
		}

		// 투자실행금
		var use_cash = u_cash * 10000;
